 ...
 */
public class DayMenuTabFragment extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, ViewGroup container,
        ...
        final View view = inflater.inflate(R.layout.fragment_daymenu_waiter, null);
    ...
    private List<Content> getDataByHeaderAndMenu(String header) {
        List<Content> list = new ArrayList<Content>();
        Content content = null;
        try {
            List<Product> listProducts = convertToContent(getProductsForMenu(header));
    ...
    private class HttpAsyncTask extends AsyncTask<String,Void,List<LinkedTreeMap>> {
        ...
        protected List<LinkedTreeMap> doInBackground(String... urls) {
            if (urls[0].contains("getallproductsinmenubytypes")) {
                try {
                    return getProductsForMenuWS(urls);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public List<LinkedTreeMap> getProductsForMenu(String type) throws ExecutionException, InterruptedException {
        String baseURL = "http://" + globalVariables.getServerIP() + ":" + globalVariables.getPort();
        return new HttpAsyncTask().execute(baseURL + "/products/getallproductsinmenubytypes?",
                type).get();
    }
